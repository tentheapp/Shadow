...
{
    CGPoint vel = [rec velocityInView:self.collectionView];
    if (vel.x > 0)
    {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    NSIndexPath *significantIndexPath;
    NSDate *currentPage = _currentPage;
    CGPoint significantPoint;
    if (_supressEvent) {
        return;
    }
    if (self.floatingMode && _collectionView.indexPathsForVisibleItems.count) {
        ...
        significantPoint = CGPointMake(_collectionView.fs_width*0.5,MIN(self.preferredRowHeight*2.75, _collectionView.fs_height*0.5)+_collectionView.contentOffset.y);
        significantIndexPath = [_collectionView indexPathForItemAtPoint:significantPoint];
        ...
        switch (_collectionViewLayout.scrollDirection) {
            case UICollectionViewScrollDirectionHorizontal: {
                ...
                NSDateComponents *dateComponents = [gregorianCalendar components:NSCalendarUnitYear| NSCalendarUnitMonth| NSCalendarUnitWeekday| NSCalendarUnitDay fromDate:currentDate];
                NSInteger present_month = [dateComponents month];
                
                NSLog(@"%ld",(long)present_month);

                
                NSDateComponents *components = [[NSCalendar currentCalendar] components:NSCalendarUnitDay | NSCalendarUnitMonth | NSCalendarUnitYear fromDate:currentPage];
                NSInteger previous_month = [components month];
                NSLog(@"%ld",(long)previous_month);


              
                
//                if (previous_month < present_month)
//                {
//                    
